 ...
 */
class DragUiData {
  constructor(name, title, ui, show, left,icon) {
    this.name = name
    this.title = title
...
let uiManage = {
  ...
  setPluginUi(vm, options) {
    options.uis.forEach(ui => {
      let left = ui.position == 'left' ? 0 : 1920 - 300
      vm.$set(this.manageUi, ui.name, new DragUiData(ui.name, ui.tabs, ui.component, ui.show, left,ui.icon));
      setTimeout(() => {
        uiFunction.updated({
          name: ui.name,
          show: ui.show,
  ...
  setPlugin(name) {
    this.manageUi[name].ui = this.manageUi[name]._ui;
  },
  setMiddleUi(vm, options) {
    // this.middleUi = new DragUiData('middle','',options.uis[0].component,options.show);
    vm.$set(this, 'middleUi', new DragUiData('middle', '', options.uis[0].component, options.show));
  },
  setMiddlePlugin() {
    this.middleUi.ui = this.middleUi._ui;
  },
  setToolBar(bar) {
    if (!bar.toolbar) return
    bar.show = false;
    upWindowTool.push(bar);
  },
}
